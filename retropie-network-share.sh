...
file_autostart="/opt/retropie/configs/all/autostart.sh"                       # Location of the autostart script
...
check_backup_file () {
  ...
  if [ ! -d "$folder_overlay_settings"/"$1"-original ]; then
    cp "$1" "$1"-original
    mv -f "$1"-original "$folder_overlay_settings"
    logger_sucess "File $file_base sucessfully backed up in folder: $folder_overlay_settings"
  else
    logger_warning "File $file does not existis in this system"
  fi
}
check_mount () {
...
command_delete_script_id () {
  ...
  sed -i "/$start/,/$end/d " "$file_autostart"
  logger_sucess "Updated script to remove script lines at: $file_autostart"
}
command_livewire () {
  # On Sed, use | on the last  as a deliniator, to avoid issues with / and folders
  start="$script_id"BEGIN
  end="$script_id"END
  if [ "$1" == "modified" ]; then
...
check_file "$file_autostart" "autostart script file" "Please check your system or re-configure inside of script."
...
select response in "Turn ON ROM network share" "Turn OFF ROM network share" "Fix network share credentials" \
  ...
  case $response in
    "Turn ON ROM network share" )
      ...
      settings_dir
      settings_credentials
      check_script_id "$file_autostart"
      command_script_id "BEGIN" "$file_autostart"
      command_mount_autostart
      command_overlay_autostart
      command_script_id "END" "$file_autostart"
      command_invert_script_id "$file_autostart"
